...
<# Begin: DataSource "AD-user-generate-table-report-failed-logons" #>
$tmpPsScript = @'
try {
    $filter = {badPwdCount -gt 2}
    $properties = "CanonicalName", "Displayname", "UserPrincipalName", "SamAccountName", "Department", "Title", "Enabled", "badPwdCount"
   ...
   Write-information "Result count: $resultCount"
    ...
    if($resultCount -gt 0){
        foreach($r in $result){
            $returnObject = @{CanonicalName=$r.CanonicalName; Displayname=$r.Displayname; UserPrincipalName=$r.UserPrincipalName; SamAccountName=$r.SamAccountName; Department=$r.Department; Title=$r.Title; Enabled=$r.Enabled; badPwdCount=$r.badPwdCount}
...
'@ 
$tmpModel = @'
[{"key":"UserPrincipalName","type":0},{"key":"Enabled","type":0},{"key":"SamAccountName","type":0},{"key":"Title","type":0},{"key":"Department","type":0},{"key":"Displayname","type":0},{"key":"badPwdCount","type":0},{"key":"CanonicalName","type":0}]
'@ 
$tmpInput = @'

'@ 
$dataSourceGuid_0 = [PSCustomObject]@{} 
Invoke-HelloIDDatasource -DatasourceName "AD-user-generate-table-report-failed-logons" -DatasourceType "4" -DatasourceInput $tmpInput -DatasourcePsScript -$tmpPsScript -DatasourceModel $tmpModel -returnObject ([Ref]$dataSourceGuid_0) 
<# End: DataSource "AD-user-generate-table-report-failed-logons" #>
<# End: HelloID Data sources #>

<# Begin: Dynamic Form "AD - Report - Accounts with failed logons" #>
$tmpSchema = @"
[{"templateOptions":{},"type":"markdown","summaryVisibility":"Show","body":"The following report will show local AD accounts that have more than 3 failed logons. Please wait while the data is loading...","requiresTemplateOptions":false},{"key":"grid","templateOptions":{"label":"Results","grid":{"columns":[{"headerName":"CanonicalName","field":"CanonicalName"},{"headerName":"Displayname","field":"Displayname"},{"headerName":"UserPrincipalName","field":"UserPrincipalName"},{"headerName":"Department","field":"Department"},{"headerName":"Title","field":"Title"},{"headerName":"Enabled","field":"Enabled"},{"headerName":"BadPwdCount","field":"badPwdCount"}],"height":500,"rowSelection":"single"},"dataSourceConfig":{"dataSourceGuid":"$dataSourceGuid_0","input":{"propertyInputs":[]}},"useFilter":true,"useDefault":false},"type":"grid","summaryVisibility":"Hide element","requiresTemplateOptions":true},{"key":"exportReport","templateOptions":{"label":"Export report (local export on HelloID Agent server)","useSwitch":true,"checkboxLabel":"Yes","mustBeTrue":true},"type":"boolean","summaryVisibility":"Show","requiresTemplateOptions":true}]
"@ 

$dynamicFormGuid = [PSCustomObject]@{} 
Invoke-HelloIDDynamicForm -FormName "AD - Report - Accounts with failed logons" -FormSchema $tmpSchema  -returnObject ([Ref]$dynamicFormGuid) 
<# END: Dynamic Form #>
...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD - Report - Accounts with failed logons" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-info-circle" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
	...
	$tmpScript = @'
            try {
                if($exportReport -eq "True") {
                    ...
                    $exportFile = $HIDreportFolder + "Report_AD_AccountsFailedLogons_" + $timeStamp + ".csv"
...
'@ 
	...
	Invoke-HelloIDAutomationTask -TaskName "AD-export-report-accounts-failed-logons" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD - Report - Accounts with failed logons' already exists. Nothing to do with the Delegated Form task..." 
}
